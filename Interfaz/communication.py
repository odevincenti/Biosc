...
ESP32S_BAUD_RATE = 115200
BUFFER_SIZE = 200

class serialESP32S(object):
    def __init__(self):
        try:
            self.serial = serial.Serial(port=self.get_port(), 
                                baudrate=ESP32S_BAUD_RATE, 
    ...
    def get_port(self):
        ...
        for port in ports:
            if 'ESP32S' in port.description:
                return port.device
        raise serial.serialutil.SerialException('ESP32S not found')


def get_data_serial(serial_connection: serialESP32S) -> bool:
